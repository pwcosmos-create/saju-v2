...
3. 剛强·刚强·家庭(한자만) 등 중국어식 혼용 표현 사용 시 번역 실패로 간주
...
7. 전체 분량 최소 6000자 이상. 각 섹션 [N]은 최소 500자 이상.
8. 각 섹션은 2~3개의 ◆ 소제목으로 구성. 소제목마다 핵심 주제 1개.
9. 소제목 형식: "◆ 소제목 텍스트" (줄 맨 앞에 ◆ 기호 + 공백). 예) "◆ 일상에서 이렇게 느껴져요"
10. ◆ 소제목 아래 본문은 반드시 2~3문단. 각 문단 3~4문장. 문단 사이 빈 줄 한 줄.
11. 표(table)·코드블록·마크다운 헤더(##) 사용 금지. 오직 **강조**·"— " 불릿·◆ 소제목만 사용.
12. 특수문자(!@#$%^&*~\`<>|\\/_+=) 사용 금지. ◆·—·**강조**·괄호() 외 기호 사용 시 오류.
13. 실존 인물(연예인·정치인·역사 인물 등) 이름을 언급하거나 "~와 같은 사주" 식의 비교 절대 금지. 이 사주는 오직 이 분만의 고유한 이야기로만 서술할 것.
14. [N] 섹션 번호는 반드시 줄 맨 앞에 단독으로 위치. **[1]** 처럼 굵게 감싸는 것 절대 금지. 올바른 예: 줄 맨 앞에 [1] 이 사주를 한 마디로 표현하면
15. ◆ 소제목은 반드시 앞에 빈 줄을 두고 줄 맨 앞에서 시작. 문장 중간에 ◆ 삽입 금지.
16. 사주 풀이 시작 전 서론·인사말·도입부 작성 금지. 반드시 [1]부터 바로 시작할 것.
━━━━━━━━━━━━━━━━━━

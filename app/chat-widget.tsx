...
const CONSULT_FETCH_MS = 600_000;
...
export default function ChatWidget({
  ...
  useEffect(() => {
    if (open && !sessionExpiresAt && canStartCounseling) {
      setSessionExpiresAt(Date.now() + 600_000); // 10분
    }
  }, [open, sessionExpiresAt, canStartCounseling]);

  useEffect(() => {
    if (!open || !sessionExpiresAt) {
      return;
    }
    const timer = setInterval(() => {
      const now = Date.now();
      const remainingSecs = Math.max(0, Math.floor((sessionExpiresAt - now) / 1000));
    ...
    <>
      ...
      <div className="saju-chat-layer" style={{
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {false && canStartCounseling && result && (
            <div
              role="note"
              style={{
                flexShrink: 0,
        ...
        {!canStartCounseling ? (
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.02)', flexShrink: 0 }}>
            ...
            {open && sessionExpiresAt && !sttTranscribing && !loading && !replyTyping && !sessionExpired && (
              <div style={{
                textAlign: 'center', padding: '4px 0', fontSize: '.75rem',
                color: timeLeft <= 60 ? '#ff6b6b' : 'rgba(255,255,255,.6)',
                fontWeight: timeLeft <= 60 ? 700 : 400
              }}>
                [남은 상담 시간: {Math.floor(timeLeft / 60).toString().padStart(2, '0')}:{(timeLeft % 60).toString().padStart(2, '0')}]
              </div>
            )}
            ...
            <div style={{
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    if (canStartCounseling && input.trim() && !sessionExpired) void send();
                  }
                }}
                placeholder={sessionExpired ? '상담 제한 시간(10분)이 종료되었습니다.' : (result ? (chatMode === 'compatibility' ? '궁합/비교 질문을 입력하세요...' : '질문을 입력하세요...') : '사주 분석 먼저 해주세요')}
                disabled={!result || !canStartCounseling || sessionExpired}
                style={{
              ...
              <button
                type="button"
                onClick={toggleVoice}
                disabled={loading || sttTranscribing || sessionExpired}
                title={
              ...
              >🎤</button>
              <button onClick={() => send()} disabled={!input.trim() || !result || !canStartCounseling || sessionExpired} style={{
                padding: '9px 14px',
        ...
        {!open && (
          <div
            ...
            style={{
              ...
              whiteSpace: 'nowrap',
              cursor: canStartCounseling ? 'pointer' : 'default',
            }}
          >

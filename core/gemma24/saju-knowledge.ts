...
import { isYearFortuneQuestion, parseYearFortuneYear, yearFortuneCardTitle } from './is-year-fortune-question';
...
function loadFromFile(filePath: string, requireConfirmed: boolean): KnowledgePack | null {
  ...
  const raw = JSON.parse(fs.readFileSync(/*turbopackIgnore: true*/ filePath, 'utf-8')) as {
    cards?: RawCard[];
    updated_at?: string;
    exported_at?: string;
  };
...
const CONSULT_TOPIC_DEEP: [RegExp, number][] = [
  ...
  [/건강|몸|질병|컨디션|체력/, 10],
  [/대운|세운|올해|내년|내녴|금년|월운|시기|흐름|언제|요즘|전망/, 6],
  [/이직|면접|승진|퇴사/, 9],
  [/결혼|재혼|이혼/, 8],
  [/성격|성향|처음|타인/, 1],
  [/오행|균형/, 4],
...
export function pickConsultDeepIds(userMessage: string): number[] {
  if (isDayFortuneQuestion(userMessage) || isYearFortuneQuestion(userMessage)) return [];

...
export const GUIDE_TEMPLATE_RE =
  /으로만\s*서술|일간\s*=\s*겉\s*성향|일주\(日柱\)\s*성향|시주\(時柱\)를\s*모르시면|본\s*내용은\s*명리\s*참고용이며\s*확정\s*예언|학파·신강신약에\s*따라\s*달라질\s*수\s*있음|인생의\s*큰\s*그림을\s*함께\s*그려보고자\s*합니다|오행\(목,\s*화,\s*토,\s*금,\s*수\)의\s*분포와\s*십신/;

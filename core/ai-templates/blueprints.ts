...
7. 각 섹션 [N]은 3~5문단으로 구성. 문단마다 핵심 주제 1개. 너무 긴 단일 문단 금지.
...
각 섹션은 독자가 자기 이야기처럼 느낄 수 있도록, 공감에서 시작해 구체적인 조언으로 마무리하는 흐름으로 써주세요.

[1] 이 사주를 한 마디로 표현하면
— "이 사주를 한 마디로 표현하면..."으로 시작. ${strengthLabel}인 이유를 어렵지 않게 풀어줄 것.
— 읽는 분이 "아, 그래서 내가 이랬구나" 하고 느낄 수 있는 한 문장으로 마무리.

[2] 이게 실생활에서는 이렇게 느껴져요 — ${strengthLabel}
— "혹시 이런 경험 있으신가요?"로 시작해 생활 밀착형 상황 3가지를 따뜻하게 서술.
${isJaedaSinyak ? '— 재물이 많은데도 늘 부족하게 느껴지는 이유, "내 돈인데 내 돈 같지 않은" 상황을 구체적으로.' : ''}

[3] 당신은 어떤 사람인가요 — ${gyeokguk}
— 강점부터 먼저. 그다음 약점을 솔직하지만 따뜻하게.
— "이런 모습, 혹시 낯익으신가요?"처럼 독자와 대화하듯 써주세요.

[4] 넘치는 것과 부족한 것 — 오행 이야기
— 과다 오행(${dominant})이 일상에서 어떻게 나타나는지 구체적 상황으로.
— 부족 오행(${lacking})을 채우는 방법을 음식·색·방향·취미 등 생활 속 예시로 제안.

[5] 나를 도와주는 에너지 — 용신·희신 활용법
— 용신(${yongsinName}): "이 에너지를 일상에서 이렇게 쓰세요" — 구체적 방법 3가지.
— 희신(${huisinNames}): 보조 역할과 활용 팁.
— 기신(${gisinNames}): "이건 조심하세요" — 실생활 예시로.

[6] 어떤 일이 잘 맞을까요 — 직업과 적성
— 잘 맞는 직종과 환경, 그 이유를 자연스럽게.
— 피하면 좋은 상황도 "이런 환경은 당신의 에너지를 빼앗아요"처럼 부드럽게.
${isJaedaSinyak ? '— 재다신약: 혼자 모든 걸 결정하기보다 시스템·파트너십이 맞는 이유.' : ''}

[7] 사람들과의 관계 — 연애·가족·친구
— 반복되는 관계 패턴을 "혹시 이런 상황 자주 겪으시나요?"로 시작.
— 배우자 인연, 주의해야 할 관계 유형을 따뜻하게.

[8] 돈과 재물 이야기
— "이 사주에 맞는 돈 버는 방식이 따로 있어요"로 시작.
— 맞는 방식과 피해야 할 방식을 현실적으로.
${isJaedaSinyak ? '— 재다신약: "감정보다 시스템" — 왜 그런지 쉽게.' : ''}

[9] ${seunYearLabel} 월별 흐름
— 각 월을 "이 달은 이런 느낌의 시기입니다" 형식으로 2~3문장. 아래 데이터 기반으로 임의 생성 금지.

【월별 엔진 데이터】
${monthlyPrompt}

[10] 앞으로 어떻게 살면 좋을까요? — 핵심 조언
— 이 사람이 평생 기억하면 좋을 한 가지 원칙과, 용신(${yongsinName})을 일상에서 쓰는 구체적인 방법.`;
}

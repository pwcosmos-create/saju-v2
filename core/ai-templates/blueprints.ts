...
사주를 처음 보는 일반인도 이해할 수 있도록, 전문 용어가 나올 때마다 쉬운 말을 먼저 쓰고 괄호에 한자를 병기합니다.
예) "일간의 기운이 약한 상태 — 신약(身弱)", "나를 도와주는 오행 — 용신(用神)"
딱딱한 강의체 대신, 읽는 사람이 자기 이야기처럼 느낄 수 있는 따뜻한 말투로 씁니다.
...
9. 전문용어 첫 등장 시 반드시 쉬운 말 먼저: "~한 상태인 신약(身弱)" 형식으로 설명 후 이후엔 용어만 사용 가능.
━━━━━━━━━━━━━━━━━━
...
아래 10가지 주제로 나눠서 따뜻하고 이해하기 쉽게 풀어주세요. 처음 사주를 접하는 분도 고개를 끄덕일 수 있을 만큼 친절하게 써주세요.

[1] 이 사주의 핵심 — 한 줄로 이해하기
— 이 사람의 사주가 어떤 구조인지 쉬운 말로 먼저 설명. 판정(${strengthLabel})이 나온 이유를 "왜 그런지"부터 풀어줄 것.

[2] ${strengthLabel} — 실생활에서 어떻게 느껴지나요?
— 전문 용어 대신 "실제로 이런 상황에서 이렇게 느껴집니다"처럼 생활 밀착형으로 서술.
${isJaedaSinyak ? '— 재물이 많은데도 늘 부족하게 느껴지는 이유를 구체적인 상황으로 묘사할 것.' : ''}

[3] 타고난 성격과 삶의 패턴 — ${gyeokguk}
— 이 사람이 어떤 사람인지, 강점과 약점을 솔직하고 따뜻하게. 자기 이야기처럼 느껴지게.

[4] 오행 불균형 — 채워야 할 것과 넘치는 것
— 과다 오행(${dominant})이 일상에서 어떤 영향을 주는지, 부족 오행(${lacking})을 어떻게 보완하면 좋은지 생활 속 방법으로.

[5] 나를 도와주는 힘 — 용신·희신 활용법
— 용신(${yongsinName}): 이 에너지를 일상에서 어떻게 쓰면 되는지 구체적으로.
— 희신(${huisinNames}): 보조 역할 설명.
— 기신(${gisinNames}): 왜 조심해야 하는지 실생활 예시로.

[6] 직업과 적성 — 어떤 일이 잘 맞을까요?
— 타고난 기질에 어울리는 직종과 환경을 이유와 함께. 피해야 할 상황도 구체적으로.
${isJaedaSinyak ? '— 재다신약: 혼자 결정하기보다 시스템·파트너십을 활용하는 방식이 맞는 이유를 쉽게 설명.' : ''}

[7] 인간관계·연애·가족
— 이 사람이 관계에서 반복하는 패턴, 배우자 인연, 조심해야 할 관계 유형을 따뜻하게 서술.

[8] 돈과 재물 — 어떻게 모아야 할까요?
— 이 사주에 맞는 재물 관리법과 맞지 않는 방식을 현실적으로.
${isJaedaSinyak ? '— 재다신약: 감정이 아닌 규칙·시스템으로 운용해야 하는 이유를 쉽게 풀어줄 것.' : ''}

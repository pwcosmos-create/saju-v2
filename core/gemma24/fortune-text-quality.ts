...
export const GUIDE_TEMPLATE_RE =
  /으로만\s*서술|일간\s*=\s*겉\s*성향|일주\(日柱\)\s*성향|시주\(時柱\)를\s*모르시면|본\s*내용은\s*명리\s*참고용이며\s*확정\s*예언|학파·신강신약에\s*따라\s*달라질\s*수\s*있음|인생의\s*큰\s*그림을\s*함께\s*그려보고자\s*합니다|오행\(목,\s*화,\s*토,\s*금,\s*수\)의\s*분포와\s*십신|귀한\s*인연으로\s*만나\s*뵙게\s*되어\s*반갑습니다|귀하의\s*사주\s*명식을\s*통해\s*타고난\s*기운과|삶의\s*큰\s*그림과\s*그\s*안에서의\s*섬세한\s*역할들을\s*함께\s*들여다보는|재물에\s*대한\s*깊은\s*고민과\s*궁금증을\s*안고\s*이\s*글을|관계\s*속에서\s*\*\*깊은\s*유대감과\s*안정감\*\*을\s*추구하는|일지가\s*보여주는\s*배우자궁의\s*특성과\s*밀접하게|직업과\s*일에\s*대한\s*깊이\s*있는\s*이야기를\s*나누게\s*되어\s*반갑습니다/;
